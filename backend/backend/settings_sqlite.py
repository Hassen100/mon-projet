...
# FORCE SQLite - ONLY SQLite, no MySQL possible
DATABASES = {
    'default': {
...
raw_cors_origins = os.getenv('CORS_ALLOWED_ORIGINS', 'http://localhost:4200,http://127.0.0.1:4200')
CORS_ALLOWED_ORIGINS = [origin.strip() for origin in raw_cors_origins.split(',') if origin.strip()]

# Google Analytics
GA_PROPERTY_ID = os.getenv('GA_PROPERTY_ID', '531307647')
ga_creds = os.getenv('GA_CREDENTIALS_JSON', '{}')
GA_CREDENTIALS = json.loads(ga_creds) if ga_creds != '{}' else {}
...
ENABLE_CREATE_ADMIN_PAGE = os.getenv('ENABLE_CREATE_ADMIN_PAGE', 'False').strip().lower() == 'true'
